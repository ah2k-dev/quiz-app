import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { Card, Spin, message, Typography, Button } from "antd";
import { clearErrors, getQuizes } from "../../actions/adminActions";
import { useNavigate } from "react-router-dom";
import BackButton from "./BackButton";
const ViewQuizes = () => {
  const { Title } = Typography;
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { error, loading, response } = useSelector(
    (state) => state.adminReducer
  );
  const btnQuiz = (data) => {
    navigate(`/admin/view-quiz/${data._id}`, { state: data });
  };
  const btnResult = (id) => {
    navigate("/admin/view-quiz-result/" + id);
  };
  useEffect(() => {
    dispatch(getQuizes());
    if (error) {
      message.error(error.message);
      dispatch(clearErrors());
    }
  }, [dispatch, error]);
  return loading ? (
    <div className="loading">
      <Spin size="large" />
    </div>
  ) : (
    <div
      className="container"
      style={{
        textAlign: "center",
        paddingTop: "10vh",
      }}
    >
      <BackButton />
      <Title>Quizes</Title>
      {response.hasOwnProperty("data") && (
        <div
          className="ant-row ant-row-center"
          style={{
            rowGap: "20px",
            columnGap: "10px",
          }}
        >
          {response.data.hasOwnProperty("quizes") ? (
            response.data.quizes.length <= 0 ? (
              <>No quizes found</>
            ) : (
              response.data.quizes.map((val, ind) => {
                return (
                  <Card
                    className="ant-col-md-6"
                    style={{
                      minWidth: "300px",
                    }}
                    key={ind}
                    title={`Quiz ${ind + 1}`}
                  >
                    <div className="vals">
                      <div>
                        <span>
                          <strong>Questions:</strong>
                        </span>
                        <span>
                          <strong>Course:</strong>
                        </span>
                        <span>
                          <strong>Max Score:</strong>
                        </span>
                        <span>
                          <strong>Time Limit:</strong>
                        </span>
                        <Button
                          type="primary"
                          style={{
                            marginTop: "20px",
                          }}
                          onClick={() => btnQuiz(val)}
                        >
                          View Questions
                        </Button>
                      </div>
                      <div>
                        <span>{val.quiz.length}</span>
                        <span>{val.course?.name || "err"}</span>
                        <span>{val.maxScore}</span>
                        <span>{val.timeLimit} min</span>
                        <Button
                          type="primary"
                          style={{
                            marginTop: "20px",
                          }}
                          onClick={() => btnResult(val._id)}
                        >
                          View Results
                        </Button>
                      </div>
                    </div>
                  </Card>
                );
              })
            )
          ) : (
            <div>No Data to show</div>
          )}
        </div>
      )}
    </div>
  );
};

export default ViewQuizes;
